...
"%wix%bin\candle.exe" *.wxs -ext WixUtilExtension -arch x64 -dPlatform=x64 -out build\x64\
"%wix%bin\light.exe" -ext WixUIExtension -ext WixUtilExtension build\x64\*.wixobj -out build\PornViewer_x64.msi -sw1076 -b node_modules
